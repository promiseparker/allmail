...
export function TodayAgenda({ events }: TodayAgendaProps) {
  ...
  // in-progress: started before now AND ends after now
  ...
  return (
    <div className="bg-white border border-border rounded-xl overflow-hidden shadow-card">
      {/* All-day chips */}
      {allDay.length > 0 && (
        <div className="px-4 py-2.5 border-b border-border bg-surface-muted flex flex-wrap gap-1.5">
          <span className="text-[10px] text-gray-400 font-medium uppercase tracking-wide self-center mr-1">
            All day
          </span>
          {allDay.map((e) => (
            <span
      ...
      {/* Now indicator — shown between past and current/upcoming */}
...
function AgendaItem({
  ...
  return (
    <div
      className={cn(
        "flex items-start gap-4 px-4 py-3 border-b border-border last:border-b-0 transition-colors hover:bg-surface-muted",
        dim && "opacity-40",
        inProgress && "bg-primary-50/50"
      )}
    >
      {/* Time */}
      <div className="w-16 flex-shrink-0 text-right">
        <p className={cn("text-xs font-medium", inProgress ? "text-primary" : "text-gray-700")}>
          {startTime}
        </p>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {event.hasConflict && (
            <span
              className={cn(
                "flex items-center gap-1 text-xs font-medium px-1.5 py-0.5 rounded flex-shrink-0",
                event.conflictSeverity === "hard"
                  ? "bg-conflict-hard-bg text-conflict-hard"
                  : "bg-conflict-soft-bg text-conflict-soft"
              )}
            >
